...
class SupabaseUsers:
    ...
    def get_eligible_auto_signal_users(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"✅ Found {len(eligible_users)} eligible auto signal users (including {len([u for u in eligible_users if u['type'] == 'admin'])} admins)")
            return eligible_users

        except Exception as e:
            logger.error(f"❌ Error getting eligible auto signal users: {e}")
            return []
