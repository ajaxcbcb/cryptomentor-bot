...
# snd_auto_signals.py - AutoSignal System for CryptoMentor AI Bot
import asyncio
import logging
from datetime import datetime
...
class AutoSignalScanner:
    """Auto Signal Scanner for CryptoMentor AI Bot"""
    
    def __init__(self, bot_instance):
        self.bot_instance = bot_instance
        self.is_running = False
        self.target_symbols = [
            'BTC', 'ETH', 'BNB', 'SOL', 'ADA', 'DOGE', 'AVAX', 'MATIC', 
            'DOT', 'LINK', 'UNI', 'ATOM', 'XRP', 'LTC', 'BCH', 'NEAR',
            'ALGO', 'VET', 'FTM', 'MANA', 'SAND', 'APE', 'CRV', 'AAVE', 'COMP'
        ]
        self.scan_interval = max(1800, 300)  # Minimum 30 minutes (1800 sec), default 5 min for testing
    ...
    async def start_auto_scanner(self):
        ...
        self.is_running = True
        logger.info("🚀 Auto Signal Scanner started")
    ...
    async def stop_auto_scanner(self):
        ...
        logger.info("🛑 Auto Signal Scanner stopped")
    ...
    async def _perform_scan(self):
        ...
        try:
            ...
            if current_time - self.last_signal_sent_time < self.signal_cooldown:
                logger.info("Signal cooldown active, skipping scan")
                return
            ...
            if not eligible_users:
                logger.info("No eligible users for auto signals")
                return
            
            # Mock signal generation (replace with actual logic)
            signal_data = await self._generate_mock_signal()
            
            if signal_data and signal_data.get('confidence', 0) >= self.min_confidence:
                await self._send_signal_to_users(signal_data, eligible_users)
                self.last_signal_sent_time = current_time
            
            self.last_scan_time = current_time
            
        except Exception as e:
            logger.error(f"Error in scan performance: {e}")
    ...
    def _get_eligible_users(self) -> List[int]:
        ...
        try:
            if hasattr(self.bot_instance, 'db') and self.bot_instance.db:
                return self.bot_instance.db.get_eligible_auto_signal_users()
            else:
                # Fallback to admin IDs only
                return list(self.bot_instance.admin_ids) if hasattr(self.bot_instance, 'admin_ids') else []
        except Exception as e:
            logger.error(f"Error getting eligible users: {e}")
            return []

    async def _generate_mock_signal(self) -> Dict[str, Any]:
        """Generate mock signal (replace with actual signal logic)"""
        import random
        
        symbol = random.choice(self.target_symbols)
        ...
        return {
            ...
            'stop_loss': round(random.uniform(19000, 69000), 2),
            'take_profit': round(random.uniform(21000, 71000), 2),
            'timestamp': datetime.now().isoformat()
        }

    async def _send_signal_to_users(self, signal_data: Dict[str, Any], user_ids: List[int]):
        """Send signal to eligible users"""
        try:
            ...
            logger.info(f"✅ Auto signal sent to {sent_count}/{len(user_ids)} users")
            
        except Exception as e:
            logger.error(f"Error sending signals: {e}")

    def _format_signal_message(self, signal_data: Dict[str, Any]) -> str:
        """Format signal into message"""
        direction_emoji = "🟢" if signal_data['direction'] == 'LONG' else "🔴"
        
        return f"""🎯 **AUTO SIGNAL ALERT** {direction_emoji}

💰 **{signal_data['symbol']}/USDT {signal_data['direction']}**
...
💡 **Auto Signal** - Admin & Lifetime Only
...
def initialize_auto_signals(bot_instance):
    ...
    try:
        ...
        logger.error(f"Failed to initialize auto signals: {e}")
        return None
