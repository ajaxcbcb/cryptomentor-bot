...
def get_risk_per_trade(telegram_id: int) -> float:
    ...
    Returns:
        Risk percentage as float in range 0.25 - 10.0.
        Default: 3.0 if not set
    """
    try:
        s = _client()
        ...
        if res.data and len(res.data) > 0:
            ...
            if stored_value is not None:
                ...
                risk_value = max(0.25, min(10.0, risk_value))
                # Log to help debug
                import logging
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.info(f"[RiskFetch:{telegram_id}] No risk_per_trade found, using default 3.0")
        return 3.0
    except Exception as e:
        import logging
        logger = logging.getLogger(__name__)
...
def set_risk_per_trade(telegram_id: int, risk_pct: float) -> Dict[str, Any]:
    ...
    Args:
        telegram_id: User's Telegram ID
        risk_pct: Risk percentage (0.25 - 10.0)
    ...
    try:
        ...
        if risk_pct < 0.25 or risk_pct > 10.0:
            return {
                'success': False,
                'error': 'Risk must be between 0.25% and 10%',
                'risk_per_trade': 0
            }
